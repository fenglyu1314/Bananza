...
本能力定义《Bananza》主角在地面沙盒场景中的**基础移动能力**、**跟随相机表现**、**输入接入方式**以及**模块落位约束**。它覆盖从玩家输入到角色位移、朝向反馈、相机平滑跟随的最小闭环，供后续跳跃 / 冲刺 / 战斗等 change 叠加扩展。

本 capability MUST 与 [`gameplay-vision`](../gameplay-vision/spec.md) 的"轻松无压力"基调保持一致，MUST 遵循 [`project-layout`](../project-layout/spec.md) 的目录与命名空间约束。
